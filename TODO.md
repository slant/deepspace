...
- [ ] **Engine Repair upgrade effect**: when unlocked, allow moving 2 spaces per turn instead of 1. No longer blocked — `Campaign#upgrade_researched?("engine_repair")` exists (see R&D section below) — just not yet wired into movement.
- [ ] **Jump Drive** (requires Engine Repair upgrade): costs 1 fuel. No longer blocked on data — `CrewDice`/R&D both exist now — just not yet wired. Two use contexts:
...
- [ ] **Long Range Scanners upgrade effect**: during Open Space roll, reduce the d6 result by 2 (min 1). No longer blocked — `Campaign#upgrade_researched?("lr_scanners")` exists — just not yet wired into the Open Space roll.
