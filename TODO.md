...
- [ ] **Long Range Scanners upgrade effect**: during Open Space roll, reduce the d6 result by 2 (min 1). Blocked on R&D existing as a way to ever unlock it.
...
- [x]/[ ] **Story flags**: officer-attribute flags (54-A's MISTRUSTING, 30-B's RESTLESS, 51-D's LUCKY) are done — see Officer State section. **Still open, BLOCKED**: event 57-A's "you may now research the Cloaking Device... cross out the '?' box" is a pure R&D-eligibility flag with no gameplay meaning until R&D exists (blocked on crew dice data, see above).
